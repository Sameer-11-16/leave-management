...
export default function Register() {
  ...
  const handleSubmit = async (e) => {
    ...
    setLoading(true);
    try {
      // Step 1: Send OTP to verification email
      await API.post('/auth/send-otp', { email: form.email });
      
      // Step 2: Store data and navigate to separate OTP page
      ...
      navigate('/verify-registration', { state: registrationData });
    } catch (err) {
      toast.error(err.response?.data?.msg || 'Failed to send OTP');
    } finally {
      setLoading(false);
  ...
  const iconField = (icon, children) => (
    <div className="input-icon-wrapper">
      <span className="input-icon" style={{ height: '52px' }}>{icon}</span>
      {children}
    </div>
  );
    ...
    <div className="auth-page">
      <div className="auth-aside">
        <div className="auth-aside-content">
          <h1 className="auth-aside-title">Join Our Growing Community</h1>
          <p className="auth-aside-text">Set up your profile in seconds and start managing your leaves with ease. We've simplified the process so you can focus on what matters most.</p>
        </div>
      </div>

      <div className="auth-form-side" style={{ padding: '20px' }}>
        <div className="auth-card register-card-wide anim-scaleIn">
          <div className="auth-header">
            ...
            <p className="auth-sub">Create your profile to get started</p>
          </div>

          <form onSubmit={handleSubmit} className="auth-form-row-responsive">
            ...
            <div className="form-group" style={{ marginBottom: '0px' }}>
              ...
              {iconField(
                ...
                <select id="reg-dept" className="form-select" value={form.department}
                  ...
                  style={{ paddingLeft: '44px', width: '100%', height: '52px' }}>
            ...
            <div className="full-width-span">
              <button className="btn btn-primary btn-full btn-lg" type="submit" disabled={loading} style={{ position: 'relative', marginTop: '12px', height: '52px' }}>
                {loading ? (
          ...
          <p className="auth-footer">Already have an account? <Link to="/login" style={{ fontWeight: '600', color: 'var(--primary)' }}>Sign in</Link></p>
        </div>
      </div>
    </div>
  );
